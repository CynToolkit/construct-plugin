declare global {
  ...
  var pipelabEngine: "electron" | "tauri";
}

type MyAction<
  ACE_CATEGORIES extends string,
  NAME extends string,
  PARAMS,
> = import("c3ide2-types").Action<ACE_CATEGORIES> & {
  forward: `_${NAME}`;
  params: PARAMS;
};
type MyCondition<
  ...
  NAME extends string,
  PARAMS,
> = import("c3ide2-types").Condition<ACE_CATEGORIES> & {
  forward: `_${NAME}`;
  params: PARAMS;
};
type MyExpression<
  ...
  NAME extends string,
  PARAMS,
> = import("c3ide2-types").Expression<ACE_CATEGORIES> & {
  forward: `_${NAME}`;
  params: PARAMS;
};
...
type ActionParamToType<T extends ActionParam> =
  any; /* T["type"] extends "string"
    ? string
...
export type IsFullScreenState = ActionParamToType<
  GetCndsParams<"_IsFullScreen">["0"]
>;
...
export type ACEGeneratorResult<NAME extends string, DATA> = {
  ...
  actions: {
    [index in `${NAME}Sync` | `${NAME}`]: MyAction<
      Categories,
      `${NAME}Sync` | `${NAME}`,
      DATA["conditions"][index]["params"]
    >;
  };
  conditions: {
    [index in
  ...
  expressions: {
    [index in `${NAME}Error` | `${NAME}Result`]: MyExpression<
      Categories,
      `${NAME}Error` | `${NAME}Result`,
      DATA["conditions"][index]["params"]
    >;
  };
  test: {
    [index in `A${NAME}A`]: MyCondition<
      Categories,
      `A${NAME}A`,
      DATA["conditions"][index]["params"]
    >;
...
export type ACEGenerator<NAME extends string> = (
  data: ACEGeneratorParam<NAME>,
) => ACEGeneratorResult<NAME>;
